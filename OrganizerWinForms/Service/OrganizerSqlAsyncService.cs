...
{
    public class OrganizerSqlAsyncService : IManagerAsyncService<OrganizerItem>
    {
        ...
        public async Task AddItemAsync(OrganizerItem item)
        {
            _context.Items.Add(item);
            await _context.SaveChangesAsync(); // Writes to SQL
            await RefreshBindingAsync();
        }
        public async Task UpdateItemAsync(OrganizerItem item)
        {
        ...
        public async Task DeleteItemAsync(OrganizerItem item)
        {
            var existing = _context.Items.Find(item.Id);
            if (existing != null)
            {
        ...
        public async Task<IEnumerable<OrganizerItem>> GetItemsAsync() => await _context.Items.AsNoTracking().ToListAsync();
        public async Task ApplySieveAsync(Models.ItemSieveDTO sieve)
        {
            ...
            {
                bool isAsc = Convert.ToBoolean((Models.ItemSortOrder)sieve.SortByTime.Value);
            ...
            {
                bool isAsc = Convert.ToBoolean((Models.ItemSortOrder)sieve.SortByEndTime.Value);
                // OrderBy needs a value, so we treat non-events as DateTime.MaxValue to push them to the end
                query = isAsc
                    ? query.OrderBy(i => (i as Models.EventItem).EndTime ?? DateTime.MaxValue)
                    : query.OrderByDescending(i => (i as Models.EventItem).EndTime ?? DateTime.MinValue);
            }
            // --- 3. APPLY TO UI ---
